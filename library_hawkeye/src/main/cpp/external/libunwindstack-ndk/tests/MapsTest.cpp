...
namespace unwindstack {
    ...
    ASSERT_EQ(PROT_READ, info
    ->flags);
    ASSERT_EQ("fake_map", info->name);
    ASSERT_EQ(0U, info->elf_offset);
    ASSERT_EQ(0U, info->load_bias.
...
EXPECT_EQ(PROT_READ
| PROT_WRITE | PROT_EXEC, info.flags);
...
ASSERT_TRUE(info
!= nullptr) << "Failed verifying index " +
std::to_string(i);
EXPECT_EQ(i
* 4096, info->start) << "Failed verifying index " +
std::to_string(i);
EXPECT_EQ((i
+ 1) * 4096, info->end) << "Failed verifying index " +
std::to_string(i);
EXPECT_EQ(0U, info->offset) << "Failed verifying index " +
std::to_string(i);
if (overlap1_index != 0 && i == overlap1_index) {
EXPECT_EQ(overlap1_name, info
->name) << "Failed verifying overlap1 name " +
std::to_string(i);
} else if (overlap2_index != 0 && i == overlap2_index) {
EXPECT_EQ(overlap2_name, info
->name) << "Failed verifying overlap2 name " +
std::to_string(i);
} else {
EXPECT_EQ("", info->name) << "Failed verifying index " +
std::to_string(i);
}
}
}
...
MapInfo *info = maps.Get(i);
EXPECT_EQ(start
+ i * 4096, info->start) << "Failed at map " +
std::to_string(i);
EXPECT_EQ(start
+ (i + 1) * 4096, info->end) << "Failed at map " +
std::to_string(i);
