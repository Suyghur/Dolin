...
namespace unwindstack {
    ...
    this->memory_.
    SetMemory(offset, &sym,
    sizeof(sym));

    std::string fake_name("fake_function");
    this->memory_.SetMemory(0x2040, fake_name.
    ...
    ASSERT_TRUE(symbols
    .GetName<TypeParam>(0x5000, 0, &this->memory_, &name, &func_offset));
    ...
    ASSERT_TRUE(symbols
    .GetName<TypeParam>(0x500f, 0, &this->memory_, &name, &func_offset));
    ASSERT_EQ("fake_function", name);
    ASSERT_EQ(0xfU, func_offset);

    // Check one before and one after the function.
    ASSERT_FALSE(symbols
    .GetName<TypeParam>(0x4fff, 0, &this->memory_, &name, &func_offset));
    ASSERT_FALSE(symbols
    .GetName<TypeParam>(0x5010, 0, &this->memory_, &name, &func_offset));
}
...
this->memory_.SetMemory(0x2100, fake_name.
...
this->memory_.SetMemory(0x2230, fake_name.
...
uint64_t func_offset;
ASSERT_TRUE(symbols
.GetName<TypeParam>(0x3005, 0, &this->memory_, &name, &func_offset));
...
ASSERT_TRUE(symbols
.GetName<TypeParam>(0x5004, 0, &this->memory_, &name, &func_offset));
...
ASSERT_TRUE(symbols
.GetName<TypeParam>(0xa011, 0, &this->memory_, &name, &func_offset));
...
ASSERT_TRUE(symbols
.GetName<TypeParam>(0x5008, 0, &this->memory_, &name, &func_offset));
ASSERT_EQ("function_one", name);
...
ASSERT_TRUE(symbols
.GetName<TypeParam>(0x3008, 0, &this->memory_, &name, &func_offset));
ASSERT_EQ("function_two", name);
ASSERT_EQ(4U, func_offset);
...
ASSERT_TRUE(symbols
.GetName<TypeParam>(0xa01a, 0, &this->memory_, &name, &func_offset));
ASSERT_EQ("function_three", name);
...
uint64_t func_offset;
ASSERT_TRUE(symbols
.GetName<TypeParam>(0x5004, 0x1000, &this->memory_, &name, &func_offset));
ASSERT_EQ("fake_function", name);
ASSERT_EQ(4U, func_offset);
...
std::string fake_name("fake_function");
this->memory_.SetMemory(0x20fb, fake_name.
...
ASSERT_TRUE(symbols_end_at_200
.GetName<TypeParam>(0x5000, 0, &this->memory_, &name, &func_offset));
ASSERT_EQ("fake_function", name);
ASSERT_EQ(0U, func_offset);
ASSERT_TRUE(symbols_end_at_200
.GetName<TypeParam>(0x3000, 0, &this->memory_, &name, &func_offset));
...
uint64_t func_offset;
ASSERT_FALSE(symbols
.GetName<TypeParam>(0x6000, 0, &this->memory_, &name, &func_offset));
this->memory_.
...
this->memory_.SetMemory(0xa100, fake_name.
...
this->memory_.SetMemory(0xa200, fake_name.
...
this->memory_.SetMemory(0xa300, fake_name.
...
ASSERT_TRUE(symbols
.GetName<TypeParam>(0x5001, 0, &this->memory_, &name, &func_offset));
ASSERT_EQ("first_entry", name);
ASSERT_EQ(1U, func_offset);

ASSERT_TRUE(symbols
.GetName<TypeParam>(0x2002, 0, &this->memory_, &name, &func_offset));
ASSERT_EQ("second_entry", name);
ASSERT_EQ(2U, func_offset);

ASSERT_TRUE(symbols
.GetName<TypeParam>(0x1003, 0, &this->memory_, &name, &func_offset));
...
st_shndx = SHN_COMMON;
sym.
st_info = STT_OBJECT | (STB_GLOBAL << 4);
sym.
st_name = 0x100;
this->memory_.
SetMemory(start_offset, &sym,
sizeof(sym));
this->memory_.
SetMemory(str_offset
+ 0x100, "global_0");
...
uint64_t offset;
EXPECT_TRUE(symbols
.GetGlobal<TypeParam>(&this->memory_, "global_0", &offset));
EXPECT_TRUE(symbols
.GetGlobal<TypeParam>(&this->memory_, "global_1", &offset));
EXPECT_TRUE(symbols
.GetGlobal<TypeParam>(&this->memory_, "global_0", &offset));
EXPECT_TRUE(symbols
.GetGlobal<TypeParam>(&this->memory_, "global_1", &offset));

EXPECT_FALSE(symbols
.GetGlobal<TypeParam>(&this->memory_, "function_0", &offset));
EXPECT_FALSE(symbols
.GetGlobal<TypeParam>(&this->memory_, "function_1", &offset));

std::string name;
EXPECT_TRUE(symbols
.GetName<TypeParam>(0x10002, 0, &this->memory_, &name, &offset));
EXPECT_EQ("function_0", name);
EXPECT_EQ(2U, offset);

EXPECT_TRUE(symbols
.GetName<TypeParam>(0x12004, 0, &this->memory_, &name, &offset));
