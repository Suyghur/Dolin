...
ASSERT_FALSE(interface
.FindEntry(0x1000, &entry_offset));
}
...
ASSERT_FALSE(interface
.HandleType(0x1000, PT_NULL, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_LOAD, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_DYNAMIC, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_INTERP, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_NOTE, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_SHLIB, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_PHDR, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_TLS, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_LOOS, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_HIOS, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_LOPROC, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_HIPROC, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_GNU_EH_FRAME, 0));
ASSERT_FALSE(interface
.HandleType(0x1000, PT_GNU_STACK, 0));
}
...
ASSERT_TRUE(interface
.HandleType(0x1000, 0x70000001, 0));
ASSERT_EQ(0x1000U, interface.

start_offset()

);
ASSERT_EQ(100U, interface.
...
memory_.SetData32(
0x1000 + reinterpret_cast
<uint64_t>(&phdr
.p_vaddr) - reinterpret_cast
<uint64_t>(&phdr),
        phdr
.p_vaddr);
ASSERT_TRUE(interface
...
0x1000 + reinterpret_cast
<uint64_t>(&phdr
.p_memsz) - reinterpret_cast
<uint64_t>(&phdr),
        phdr
.p_memsz);
ASSERT_TRUE(interface
.HandleType(0x1000, 0x70000001, 0));
ASSERT_EQ(0x2000U, interface.

start_offset()

);
ASSERT_EQ(320U, interface.
...
// Non-zero load bias.
ASSERT_TRUE(interface
.HandleType(0x1000, 0x70000001, 0x1000));
ASSERT_EQ(0x1000U, interface.
...
bool finished;
ASSERT_FALSE(interface
.StepExidx(0x7000, 0, nullptr, nullptr, &finished));
EXPECT_EQ(ERROR_UNWIND_INFO, interface
.
...
regs.
set_sp(regs[ARM_REG_SP]);
regs.set_pc(0x1234);
ASSERT_FALSE(interface
.StepExidx(0x7000, 0, &regs, &process_memory_, &finished));
EXPECT_EQ(ERROR_MEMORY_INVALID, interface
.

LastErrorCode()

);
EXPECT_EQ(0x1004U, interface.
...
ASSERT_EQ(0x1000U, regs.

sp()

);
ASSERT_EQ(0x1000U, regs[ARM_REG_SP]);
ASSERT_EQ(0x20000U, regs.
...
ASSERT_TRUE(interface
.StepExidx(0x8000, 0x1000, &regs, &process_memory_, &finished));
EXPECT_EQ(ERROR_UNWIND_INFO, interface
.
...
// Pc too small.
ASSERT_FALSE(interface
.StepExidx(0x8000, 0x9000, &regs, &process_memory_, &finished));
EXPECT_EQ(ERROR_UNWIND_INFO, interface
.
...
ASSERT_TRUE(interface
.StepExidx(0x7000, 0, &regs, &process_memory_, &finished));
EXPECT_EQ(ERROR_NONE, interface
.
...
ASSERT_FALSE(finished);
ASSERT_EQ(0x10004U, regs.

sp()

);
ASSERT_EQ(0x10004U, regs[ARM_REG_SP]);
ASSERT_EQ(0x10U, regs.
...
ASSERT_EQ(0x10000U, regs.

sp()

);
ASSERT_EQ(0x10000U, regs[ARM_REG_SP]);
ASSERT_EQ(0x1234U, regs.
