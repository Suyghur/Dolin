...
namespace unwindstack {
    ...
    ASSERT_FALSE(dex_file
    .Open(0, "/file/does/not/exist"));
}
...
lseek(tf
.fd, 0, SEEK_SET));
ASSERT_EQ(sizeof(art::DexFile::Header), static_cast
<size_t>(TEMP_FAILURE_RETRY(write(
...
static_cast
<size_t>(TEMP_FAILURE_RETRY(write(tf.fd, kDexData, sizeof(kDexData)))
));

DexFileFromFile dex_file;
ASSERT_TRUE(dex_file
.Open(0, tf.path));
}
...
static_cast
<size_t>(TEMP_FAILURE_RETRY(write(tf.fd, kDexData, sizeof(kDexData) - 10))
));
...
dex_file.
reset(DexFile::Create(0x4000, &memory, &info)
);
ASSERT_TRUE(dex_file
== nullptr);
}
