...
//   #9 pc 0039a1ef libartd.so  art::interpreter::Execute+719
    ...
    std::string Unwinder::FormatFrame(const FrameData &frame, bool is32bit) {
        ...
        if (is32bit) {
            data += android::base::StringPrintf("  #%02zu pc %08"
            PRIx64, frame.num, frame.rel_pc);
        } else {
            data += android::base::StringPrintf("  #%02zu pc %016"
            PRIx64, frame.num, frame.rel_pc);
        }

        if (frame.map_offset != 0) {
            data += android::base::StringPrintf(" (offset 0x%"
            PRIx64
            ")", frame.map_offset);
        }
        ...
        if (frame.map_start == frame.map_end) {
            ...
            data += android::base::StringPrintf("  <anonymous:%"
            PRIx64
            ">", frame.map_start);
        }
