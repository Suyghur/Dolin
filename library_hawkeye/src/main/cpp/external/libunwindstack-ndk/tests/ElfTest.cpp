...
ASSERT_FALSE(elf
.GetFunctionName(0, &name, &func_offset));

bool finished;
ASSERT_FALSE(elf
.Step(0, 0, 0, nullptr, nullptr, &finished));
}
...
ASSERT_EQ(static_cast
<uint32_t>(EM_AARCH64), elf
.

machine_type()

);
ASSERT_EQ(ELFCLASS64, elf
.
...
TestInitGnuDebugdata<Elf32_Ehdr, Elf32_Shdr>(ELFCLASS32, EM_ARM,
false,
[&](
uint64_t offset,
...
TestInitGnuDebugdata<Elf64_Ehdr, Elf64_Shdr>(ELFCLASS64, EM_AARCH64,
false,
[&](
uint64_t offset,
const void *ptr, size_t
...
bool finished;
ASSERT_TRUE(elf
.Step(0x1000, 0x1000, 0x2000, &regs, &process_memory, &finished));
...
public:
    ...
    MOCK_METHOD5(Step,
    bool(uint64_t, uint64_t, Regs
    *, Memory*, bool*));
    MOCK_METHOD2(GetGlobalVariable,
    bool(
    const std::string&, uint64_t*));
    MOCK_METHOD1(IsValidPc,
    bool(uint64_t)
    );
...
ASSERT_TRUE(elf
.Step(0x1004, 0x1000, 0x2000, &regs, &process_memory, &finished));
}
...
ASSERT_TRUE(elf
.Step(0x7304, 0x7300, 0x2000, &regs, &process_memory, &finished));
}
...
ASSERT_FALSE(elf
.
GetGlobalVariable(global, &offset
));
}
...
elf.
FakeSetGnuDebugdataInterface(gnu_interface);
...
EXPECT_CALL(*interface, IsValidPc(0x1500)
).
WillOnce(::testing::Return(true)
);
...
EXPECT_CALL(*interface, IsValidPc(0x500)
).
WillOnce(::testing::Return(true)
);
...
EXPECT_CALL(*gnu_interface, IsValidPc(0x1500)
).
WillOnce(::testing::Return(true)
);
...
elf.
GetLastError(&error);
EXPECT_EQ(ERROR_MEMORY_INVALID, error
.code);
...
FakeSetInterface(interface);
interface->
FakeSetErrorCode(ERROR_MEMORY_INVALID);
...
EXPECT_EQ(ERROR_MEMORY_INVALID, elf
.

GetLastErrorCode()

);
EXPECT_EQ(0x1000U, elf.
