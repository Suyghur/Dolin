...
size_t i = 0;
i < 1024; i++) {
ASSERT_EQ(0x4cU, dst[i]) << "Failed at byte " <<
i;
}
...
getpagesize(), getpagesize(), PROT_NONE
...
ASSERT_EQ(0, munmap(reinterpret_cast<void *>(reinterpret_cast
<uintptr_t>(src)
+ pagesize),
pagesize));
...
memset(mapping,
0xFF, 3 * page_size);
ASSERT_EQ(0, mprotect(static_cast
<char *>(mapping)
+ page_size, page_size, PROT_NONE));
...
memset(mapping,
0xFF, 3 * page_size);
ASSERT_EQ(0, munmap(static_cast
<char *>(mapping)
+ page_size, page_size));
...
0xFC, 2 * page_size);
ASSERT_EQ(0, mprotect(static_cast
<char *>(mapping), page_size, PROT_NONE
));
