...
1024));
ASSERT_TRUE(memcmp(buffer.data(), ELFMAG, SELFMAG)
== 0);
...
ASSERT_TRUE(memory
->ReadFully(0, e_ident, SELFMAG));
ASSERT_EQ(0,
memcmp(e_ident, ELFMAG, SELFMAG
));

// Read past the end of what would normally be the size of the map.
ASSERT_TRUE(memory
->ReadFully(0x1000, e_ident, 1));
}
