...
10));
buffer[10] = '\0';
ASSERT_STREQ("abcdefghij", buffer.
...
i < 5; i++) {
expected_str += '1';
expected_str += static_cast
<char>(((i
+ pagesize) % 10) + '0');
}
ASSERT_STREQ(expected_str
.
...
ASSERT_FALSE(memory_
.
Init(tf_
->path, file_size + 2 * pagesize));
...
.
Init(tf_
->path, 2 * pagesize + pagesize / 2 + pagesize / 4));
}
...
ASSERT_FALSE(memory_
.ReadFully(10000, buffer.

data(),

10));
ASSERT_FALSE(memory_
.ReadFully(5000, buffer.

data(),

10));
ASSERT_FALSE(memory_
.ReadFully(4990, buffer.
...
ASSERT_TRUE(pagesize
> 100);
std::vector <uint8_t> buffer(pagesize - 100);
for (
size_t i = 0;
i<pagesize - 100; i++) {
buffer[i] = static_cast
<uint8_t>((i
% 0x5e) + 0x20);
}
...
i<pagesize;
i++) {
ASSERT_EQ(2, read_buffer[i]) << "Failed at byte " <<
i;
}
for (
size_t i = pagesize;
i<pagesize * 2; i++) {
ASSERT_EQ(3, read_buffer[i]) << "Failed at byte " <<
i;
}
}
...
.
Init(tf_
->path, pagesize + 0x100, pagesize * 2));
...
for (
size_t i = pagesize - 0x100;
i < 2 * pagesize - 0x100; i++) {
ASSERT_EQ(3, read_buffer[i]) << "Failed at byte " <<
i;
}
for (
size_t i = 2 * pagesize - 0x100;
i<pagesize * 2; i++) {
ASSERT_EQ(4, read_buffer[i]) << "Failed at byte " <<
i;
}
}
...
.
Init(tf_
->path, pagesize + 0x100, UINT64_MAX));

std::vector <uint8_t> read_buffer(pagesize * 10);
ASSERT_FALSE(memory_
.
ReadFully(pagesize
* 9 - 0x100 + 1, read_buffer.
...
i<pagesize;
i++) {
ASSERT_EQ(1, read_buffer[i]) << "Failed at byte " <<
i;
}
