...
sizeof(ehdr)));
for (
size_t i = sizeof(ehdr);
i<buffer.

size();

i++) {
ASSERT_EQ(0, buffer[i]) << "Failed at byte " <<
i;
}
...
memcpy(&buffer[info.offset], &ehdr,
sizeof(ehdr));
ASSERT_TRUE(android::base::WriteFully(elf_.fd, buffer.data(), buffer.size())
);
...
ASSERT_FALSE(elf
->memory()->ReadFully(0x1000, buffer.
...
e_shoff = 0x2000;
ehdr.
e_shentsize = sizeof(Elf32_Shdr) + 100;
ehdr.
e_shnum = 4;
memcpy(&buffer[info.offset], &ehdr,
sizeof(ehdr));
...
e_shoff = 0x2000;
ehdr.
e_shentsize = sizeof(Elf64_Shdr) + 100;
ehdr.
e_shnum = 4;
memcpy(&buffer[info.offset], &ehdr,
sizeof(ehdr));
...
i<kNumConcurrentThreads;
i++) {
EXPECT_EQ(elf, elf_in_threads[i]
) << "Thread " << i << " mismatched.";
}
}
