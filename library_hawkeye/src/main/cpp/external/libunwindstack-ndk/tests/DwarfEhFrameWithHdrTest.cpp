...
// NOTE: All test class variables need to be referenced as this->.
    ...
    0x1000, std::vector <uint8_t> {
    0x1, DW_EH_PE_udata2, DW_EH_PE_udata4, DW_EH_PE_sdata4
}
);
...
EXPECT_EQ(1U, this->eh_frame_->

TestGetVersion()

);
EXPECT_EQ(DW_EH_PE_udata2,
this->eh_frame_->

TestGetPtrEncoding()

);
EXPECT_EQ(DW_EH_PE_sdata4,
this->eh_frame_->

TestGetTableEncoding()

);
EXPECT_EQ(4U, this->eh_frame_->

TestGetTableEntrySize()

);
EXPECT_EQ(126U, this->eh_frame_->

TestGetFdeCount()

);
EXPECT_EQ(0x500U, this->eh_frame_->

TestGetPtrOffset()

);
EXPECT_EQ(0x100aU, this->eh_frame_->

TestGetEntriesOffset()

);
EXPECT_EQ(0x1100U, this->eh_frame_->

TestGetEntriesEnd()

);
EXPECT_EQ(0x1000U, this->eh_frame_->

TestGetEntriesDataOffset()

);
EXPECT_EQ(0x100aU, this->eh_frame_->

TestGetCurEntriesOffset()
...
this->eh_frame_->
TestSetTableEncoding(DW_EH_PE_udata4);
...
this->eh_frame_->
TestSetTableEncoding(DW_EH_PE_pcrel
| DW_EH_PE_udata4);
this->eh_frame_->TestSetEntriesOffset(0x1000);
...
this->eh_frame_->
TestSetTableEncoding(DW_EH_PE_datarel
| DW_EH_PE_udata4);
this->eh_frame_->TestSetEntriesOffset(0x1000);
this->eh_frame_->TestSetEntriesDataOffset(0x3000);
this->eh_frame_->TestSetTableEntrySize(0x10);
...
pc = 0x1000 * (i + 1);
info.
offset = 0x5000 + i * 0x20;
this->eh_frame_->
TestSetFdeInfo(i, info
);
}
...
EXPECT_TRUE(this->eh_frame_->
GetFdeOffsetBinary(pc, &fde_offset,
10)) << "Failed at index " <<
i;
EXPECT_EQ(0x5000 + i * 0x20, fde_offset) << "Failed at index " <<
i;
EXPECT_TRUE(this->eh_frame_->
GetFdeOffsetBinary(pc
+ 1, &fde_offset, 10))
<< "Failed at index " <<
i;
EXPECT_EQ(0x5000 + i * 0x20, fde_offset) << "Failed at index " <<
...
EXPECT_EQ(DW_EH_PE_sdata4, fde
->cie->fde_address_encoding);
EXPECT_EQ(DW_EH_PE_omit, fde
->cie->lsda_encoding);
EXPECT_EQ(0U, fde->cie->segment_size);
EXPECT_EQ(1U, fde->cie->augmentation_string.
