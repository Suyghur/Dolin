...
namespace unwindstack {
    ...
    dex_files_->
    GetMethodInformation(maps_
    .

    get(), info,

    0, &method_name, &method_offset);
...
0x300100, &method_name, &method_offset);
...
0x301102, &method_name, &method_offset);
EXPECT_EQ("Main.<init>", method_name);
...
0x300104, &method_name, &method_offset);
EXPECT_EQ("Main.<init>", method_name);
...
0x300106, &method_name, &method_offset);
EXPECT_EQ("Main.<init>", method_name);
...
dex_files_.reset(new
DexFiles(process_memory_, libs
));
dex_files_->
SetArch(ARCH_ARM);
