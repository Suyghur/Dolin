...
namespace unwindstack {
    ...
    public:
        ...
        MOCK_METHOD4(Log,
        bool(uint8_t, uint64_t, uint64_t,
        const DwarfFde*));

        MOCK_METHOD5(Eval,
        bool(
        const DwarfCie*, Memory*, const dwarf_loc_regs_t&, Regs*, bool*));

        MOCK_METHOD3(GetCfaLocationInfo,
        bool(uint64_t,
        const DwarfFde*, dwarf_loc_regs_t*));
        ...
        *));
        ...
        MOCK_METHOD1(GetFdeFromOffset,
        const DwarfFde *(uint64_t)
        );
        ...
        MOCK_METHOD1(IsCie32,
        bool(uint32_t)
        );

        MOCK_METHOD1(IsCie64,
        bool(uint64_t)
        );
    ...
    ASSERT_TRUE(mock_section
    .GetFdeFromPc(0x1000) == nullptr);
}
...
EXPECT_CALL(mock_section, GetFdeFromOffset(::testing::_)
).
WillOnce(::testing::Return(&fde)
);
...
bool finished;
ASSERT_FALSE(mock_section
.Step(0x1000, nullptr, nullptr, &finished));
}
...
bool finished;
ASSERT_TRUE(mock_section
.Step(0x1000, nullptr, &process, &finished));
}
...
.Step(0x1000, nullptr, &process, &finished));
ASSERT_TRUE(mock_section
.Step(0x1500, nullptr, &process, &finished));
}
...
ASSERT_TRUE(mock_section
.Step(0x600, nullptr, &process, &finished));
ASSERT_TRUE(mock_section
.Step(0x700, nullptr, &process, &finished));
}
