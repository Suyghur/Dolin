...
<< "Timed out waiting for remote process to be ready.";
...
0, 0))
<< "ptrace detach failed with unexpected error: " <<
strerror(errno);
}
...
ASSERT_EQ(0,
syscall(__NR_tgkill, getpid(), tid
.
...
ASSERT_TRUE(ucontext
!= nullptr) << "Timed out waiting for thread to respond to signal.";
