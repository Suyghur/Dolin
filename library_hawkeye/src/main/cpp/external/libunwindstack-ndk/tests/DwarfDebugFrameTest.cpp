...
// NOTE: All test class variables need to be referenced as this->.
    ...
    ASSERT_EQ(4U, this->debug_frame_->
...
ASSERT_TRUE(this->debug_frame_->Init(0x5000, 0x600));
ASSERT_EQ(2U, this->debug_frame_->
...
this->memory_.SetMemory(0x5010, std::vector <uint8_t>{
0x81, 0x80, 0x80, 0x00});
// Return address register
this->memory_.SetData8(0x5014, 0x84);
// Augmentation length
this->memory_.SetMemory(0x5015, std::vector <uint8_t>{
0x84, 0x00});
// R data.
...
ASSERT_EQ(1U, this->debug_frame_->
...
0x80, 0x00});
// Data alignment factor.
this->memory_.SetMemory(0x5012, std::vector <uint8_t>{
0x81, 0x80, 0x80, 0x00});
// Return address register
this->memory_.SetMemory(0x5016, std::vector <uint8_t>{
0x85, 0x10});
// Augmentation length
this->memory_.SetMemory(0x5018, std::vector <uint8_t>{
0x84, 0x00});
// L data.
...
i;
ASSERT_TRUE(this->debug_frame_->
GetFdeOffsetFromPc(pc
+ 1, &fde_offset)) << "Failed at index "
<<
i;
EXPECT_EQ(0x5000 + i * 0x20, fde_offset) << "Failed at index " <<
i;
...
EXPECT_EQ(0x5000 + i * 0x20, fde_offset) << "Failed at index " <<
i;
ASSERT_FALSE(this->debug_frame_->
GetFdeOffsetFromPc(pc
+ 0xfff, &fde_offset))
<< "Failed at index " <<
i;
