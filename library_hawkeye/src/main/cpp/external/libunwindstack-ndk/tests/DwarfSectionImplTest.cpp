...
namespace unwindstack {
    ...
    public:
        ...
        MOCK_METHOD2(GetFdeOffsetFromPc,
        bool(uint64_t, uint64_t
        *));

        MOCK_METHOD1(GetFdeFromIndex,
        const DwarfFde *(size_t)
        );
...
// NOTE: All test class variables need to be referenced as this->.
    ...
    regs[5] = 0x20;
    regs[9] = 0x3000;
    loc_regs[CFA_REG] = DwarfLocation {
    DWARF_LOCATION_VAL_EXPRESSION, {
    0x2, 0x5002
}
};
bool finished;
ASSERT_FALSE(this->section_->
Eval(&cie, &
this->memory_, loc_regs, &regs, &finished));
EXPECT_EQ(DWARF_ERROR_MEMORY_INVALID,
this->section_->

LastErrorCode()

);
EXPECT_EQ(0x5000U, this->section_->
...
Eval(&cie, &
this->memory_, loc_regs, &regs, &finished));
EXPECT_EQ(DWARF_ERROR_ILLEGAL_STATE,
this->section_->
...
0x4, 0x5004}};
bool finished;
ASSERT_TRUE(this->section_->
Eval(&cie, &
this->memory_, loc_regs, &regs, &finished));
ASSERT_FALSE(finished);
EXPECT_EQ(0x80000000U, regs.

sp()

);
EXPECT_EQ(0x20U, regs.
...
Eval(&cie, &
this->memory_, loc_regs, &regs, &finished));
EXPECT_EQ(DWARF_ERROR_NOT_IMPLEMENTED,
this->section_->
...
Eval(&cie, &
this->memory_, loc_regs, &regs, &finished));
EXPECT_EQ(DWARF_ERROR_CFA_NOT_DEFINED,
this->section_->
...
DWARF_LOCATION_REGISTER, {
8, 0}};
loc_regs[1] = DwarfLocation{
DWARF_LOCATION_REGISTER, {
3, 1}};
loc_regs[9] = DwarfLocation{
DWARF_LOCATION_REGISTER, {
1, 2}};
bool finished;
ASSERT_TRUE(this->section_->
...
DWARF_LOCATION_REGISTER, {
0, 1}};
loc_regs[2] = DwarfLocation{
DWARF_LOCATION_REGISTER, {
1, 2}};
loc_regs[3] = DwarfLocation{
DWARF_LOCATION_REGISTER, {
2, 3}};
loc_regs[4] = DwarfLocation{
DWARF_LOCATION_REGISTER, {
3, 4}};
loc_regs[5] = DwarfLocation{
DWARF_LOCATION_REGISTER, {
4, 5}};
bool finished;
ASSERT_TRUE(this->section_->
...
this->memory_, loc_regs, &regs, &finished));
EXPECT_FALSE(finished);
EXPECT_EQ(0x10U, regs.

pc()

);
EXPECT_EQ(0x2100U, regs.
...
EXPECT_EQ(0x4000U, this->section_->
...
EXPECT_EQ(DW_EH_PE_sdata4, cie
->fde_address_encoding);
EXPECT_EQ(DW_EH_PE_omit, cie
->lsda_encoding);
EXPECT_EQ(0U, cie->segment_size);
EXPECT_EQ(1U, cie->augmentation_string.
...
EXPECT_EQ(DWARF_ERROR_UNSUPPORTED_VERSION,
this->section_->
...
this->memory_.SetData8(0x5009, '\0');
this->memory_.SetData8(0x500a, 4);
this->memory_.SetMemory(0x500b, std::vector <uint8_t>{
...
this->memory_.SetData32(0x5004, 0xffffffff);
this->memory_.SetData8(0x5008, 0x1);
this->memory_.SetMemory(0x5009, std::vector <uint8_t>{
'z', 'L', 'P', 'R', '\0'});
...
!= nullptr);
EXPECT_EQ(1U, cie->version);
EXPECT_EQ(DW_EH_PE_udata2, cie
->fde_address_encoding);
EXPECT_EQ(DW_EH_PE_textrel
| DW_EH_PE_udata2, cie->lsda_encoding);
EXPECT_EQ(0U, cie->segment_size);
EXPECT_EQ(5U, cie->augmentation_string.
...
this->memory_.SetData8(0x500a, 4);
this->memory_.SetData8(0x500b, 8);
this->memory_.SetMemory(0x500c, std::vector <uint8_t>{
0x81, 0x03});
...
this->memory_.SetMemory(0x500e, std::vector <uint8_t>{
0x81, 0x03});
...
EXPECT_CALL(*this->section_, GetCieOffsetFromFde32(0x8000)).
WillOnce(::testing::Return(0x8000)
);
DwarfCie cie{};
cie.
fde_address_encoding = DW_EH_PE_udata4;
this->section_->TestSetCachedCieEntry(0x8000, cie);
EXPECT_CALL(*this->section_, AdjustPcFromFde(0x5000)).
WillOnce(::testing::Return(0x5000)
);
...
this->memory_.SetData32(0x4008, 0x5000);
this->memory_.SetData32(0x400c, 0x100);
this->memory_.SetMemory(0x4010, std::vector <uint8_t>{
0x82, 0x01});
...
DwarfFde fde_cached{};
fde_cached.
cfa_instructions_offset = 0x1000;
fde_cached.
cfa_instructions_end = 0x1100;
fde_cached.
pc_start = 0x9000;
fde_cached.
pc_end = 0x9400;
...
DwarfCie cie{};
cie.
cfa_instructions_offset = 0x3000;
cie.
cfa_instructions_end = 0x3002;
DwarfFde fde{};
fde.
cie = &cie;
fde.
cie_offset = 0x8000;
fde.
cfa_instructions_offset = 0x6000;
fde.
cfa_instructions_end = 0x6002;

this->memory_.SetMemory(0x3000, std::vector <uint8_t>{
0x09, 0x02, 0x01});
this->memory_.SetMemory(0x6000, std::vector <uint8_t>{
0x09, 0x04, 0x03});
...
ASSERT_NE(entry, loc_regs
.

end()

);
ASSERT_EQ(DWARF_LOCATION_REGISTER, entry
->second.type);
...
DwarfCie cie{};
cie.
cfa_instructions_offset = 0x5000;
cie.
cfa_instructions_end = 0x5001;
DwarfFde fde{};
fde.
cie = &cie;
fde.
