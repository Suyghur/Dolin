...
namespace unwindstack {
    ...
    // Memory error.
    ASSERT_FALSE(this->op_->

    Decode()

    );
    ASSERT_EQ(DWARF_ERROR_MEMORY_INVALID,
    this->op_->

    LastErrorCode()

    );
    EXPECT_EQ(0U, this->op_->
    ...
    // No error.
    this->op_memory_.SetMemory(0, std::vector <uint8_t> {
    0x96
}
);
this->mem_->set_cur_offset(0);
ASSERT_TRUE(this->op_->

Decode()

);
ASSERT_EQ(DWARF_ERROR_NONE,
this->op_->

LastErrorCode()

);
ASSERT_EQ(0x96U, this->op_->

cur_op()

);
ASSERT_EQ(1U, this->mem_->
...
ASSERT_EQ(8U, this->mem_->

cur_offset()

);
ASSERT_EQ(4U, this->op_->
...
ASSERT_EQ(DWARF_ERROR_TOO_MANY_ITERATIONS,
this->op_->
...
ASSERT_EQ(DWARF_ERROR_NOT_IMPLEMENTED,
this->op_->
...
ASSERT_EQ(DWARF_ERROR_STACK_INDEX_NOT_VALID,
this->op_->
...
ASSERT_EQ(0x12345678U, this->op_->
...
for (
size_t i = 1;
i < sizeof(TypeParam); i++) {
this->op_memory_.SetMemory(
0, std::vector <uint8_t>{
0x0a, 0x10, 0x20, 0x94, static_cast
<uint8_t>(i)
});
ASSERT_TRUE(this->op_->Eval(0, 5)) << "Failed at size " <<
i;
ASSERT_EQ(1U, this->op_->
...
this->op_->StackAt(0)) << "Failed at size " <<
i;
}

// Zero byte read.
this->op_memory_.SetMemory(0, std::vector <uint8_t>{
0x0a, 0x10, 0x20, 0x94, 0x00});
...
// Read too many bytes.
this->op_memory_.SetMemory(0, std::vector <uint8_t>{
0x0a, 0x10, 0x20, 0x94, sizeof(TypeParam) + 1});
ASSERT_FALSE(this->op_->Eval(0, 5));
ASSERT_EQ(DWARF_ERROR_ILLEGAL_VALUE,
this->op_->
...
ASSERT_EQ(0x0a, this->op_->

cur_op()

);
ASSERT_EQ(3U, this->op_->
...
ASSERT_EQ(0x0c, this->op_->

cur_op()

);
ASSERT_EQ(5U, this->op_->
...
ASSERT_EQ(0x0e, this->op_->

cur_op()

);
ASSERT_EQ(7U, this->op_->
...
this->op_->StackAt(2));
ASSERT_EQ(expected[i + 2],
this->op_->StackAt(1));
ASSERT_EQ(expected[i + 3],
this->op_->StackAt(0));
}
}
...
) << "Failed op: 0x" << std::hex <<
op;
ASSERT_EQ(op
- 0x30U, this->op_->StackAt(0)) << "Failed op: 0x" << std::hex <<
op;
}
}
...
op;
ASSERT_EQ(op
- 0x50U, this->op_->StackAt(0)) << "Failed op: 0x" << std::hex <<
op;
}
}
...
op;
ASSERT_EQ(op
- 0x70 + 10 + 0x12, this->op_->StackAt(0)) << "Failed op: 0x" << std::hex <<
op;
offset += 2;
...
op;
ASSERT_EQ(op
- 0x70 + 10 - 2, this->op_->StackAt(0)) << "Failed op: 0x" << std::hex <<
op;
offset += 2;
}
}
