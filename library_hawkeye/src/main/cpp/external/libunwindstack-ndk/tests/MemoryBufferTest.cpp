...
101));
ASSERT_FALSE(memory_
->ReadFully(100, buffer.

data(),

1));
ASSERT_FALSE(memory_
->ReadFully(101, buffer.
...
std::vector <uint8_t> buffer(memory_->Size());
ASSERT_EQ(128U, memory_->Read(128, buffer.
...
size_t i = 0;
i < 128; i++) {
ASSERT_EQ(128 + i, buffer[i]) << "Failed at byte " <<
i;
}
}
